...
func (s *IMServiceImpl) Send(ctx context.Context, req *rpc.SendRequest) (*rpc.SendResponse, error) {
	...
	err = s.mysqlClient.SaveMessage(ctx, roomID, message)
	if err != nil {
		return nil, err
	}
...
func (s *IMServiceImpl) Pull(ctx context.Context, req *rpc.PullRequest) (*rpc.PullResponse, error) {
	...
	messages, err := s.mysqlClient.GetMessagesByRoomID(ctx, roomID, start, end, req.GetReverse())
	if err != nil {
		return nil, err
	}
...
func getRoomID(chat string) (string, error) {
	...
	sender1, sender2 := senders[0], senders[1]
	// Compare the sender and receiver alphabetically, and sort them in ascending order to form the room ID
